      ...
      <div className="bg-gray-800 w-60 flex flex-col">
        ...
        <div className="text-gray-300  flex-1 overflow-y-scroll mt-[12px] scrollbar-hide font-medium space-y-[21px]">
          {data[1].categories.map((category, index) => (
            <div key={category.id}>
              {category.label && (
                <button className="flex items-center px-0.5 text-xs font-poppins uppercase tracking-wide">
                  <Icons.Arrow className="size-3 mr-0.5" />
                  {category.label}
                </button>
              )}
...
function ChannelLink({ channel }) {
  ...
  const classes = {
    ...
    inactiveUnread: "text-white hover:bg-gray-550/[0.32]", // Changed to match the state
  ...
  return (
    <Link
      ...
      className={`${classes[state]} flex  items-center mx-2 px-2 py-1 rounded group`}
    >
